...
## رابعاً-ب: نتائج جولة التحقق المستأنفة (17:37–17:45 UTC — قبل الإيقاف بطلب المستخدم)

الاستئناف شغّل وكلاء التحقق العدائي فعلياً وأنجز معظمهم قبل الإيقاف. النتائج محفوظة في `docs/research/journal.jsonl` (الأرشيف الخام). الخلاصة:

### ⛔ ادعاء مرفوض (تصحيح جوهري للدراسة)
**«تقسيم الفئات داخل إعلان Advantage+ Catalog» (help/217594425462043) — مرفوض:**
1. المقالة تصف صيغة **عرض** (category tiles داخل إعلان واحد) وليست آلية لتشغيل ملابس وساعات كمجموعات إعلانية منفصلة.
2. **الميزة ماتت: ميتا أزالت «product categories» من Advantage+ catalog ads بحلول يوليو 2025** (الإعلانات تحولت تلقائياً لكاروسيل، بلا بديل مباشر).
3. الاستنتاج العملي (كتالوج واحد يُقسَّم منطقياً) **يبقى صحيحاً لكن عبر Product Sets فقط** — وهذا ما يجب أن تعتمده الدراسة والنظام.
⟵ **أثر على الدراسة:** أي اعتماد على «category ads» يُحذف؛ التقسيم = Product Sets حصراً.

### ✅ توثيق Product Sets الكامل (help/741923962861190 — تحقق حرفي)
- «A set, or product set, is a sub-group of items within your catalogue» — الآلية الرسمية للتقسيم داخل كتالوج واحد.
- تُبنى بفلاتر قواعد على حقول المنتج ⟵ **يمكن للنظام المؤتمت صيانة set «ساعات» وset «ملابس» برمجياً** (مثلاً فلتر على product_type أو google_product_category).
- قيود تشغيلية: لا يمكن حذف set مستخدم بحملة شغالة، ويُنصح بعدم تعديله أثناء التشغيل.
- حد أدنى: 4 عناصر لإعلانات Collection، 2 لإعلانات Carousel.
- الصلاحيات: manage catalogue أو create ads داخل نفس الـ Business Manager.

### ✅ تأكيدات إضافية من التوثيق الرسمي (developers.facebook.com — variants)
- «Each variant must have the same item_group_id defined in the product feed» — تحقق 3 أصوات.
- **«You must supply a unique image for items that differ by color, material, or pattern»** — كل تركيبة (مينا × سوار) للساعة إلزامياً بصورتها الخاصة.
- «All variants for a given product item_group_id must have populated every variant field» — لو استخدمنا size+color للساعات، كل الـ variants لازم تعبي الحقلين.
- «only one variant of each item will display in your ads and shop» — المجموعة تظهر كعنصر واحد لا كتكرارات.

### ✅ تأكيدات socioh (تحقق حرفي)
- «For most brands, one big catalog is the way to go. You can slice and dice with Product Sets... Multiple catalogs only make sense if you're [genuinely separate businesses]».
- عيب الكتالوج الواحد إبداعي فقط (لا A/B للكرييتف الكتالوجي)، ومقارنة كتالوجين ضدياً غير موثوقة أصلاً (أحدهما يحمل تعلماً تاريخياً).
- Supplementary feeds لتبديل الصور بلا فقدان تعلم.
- نفس الكتالوج بالـ Shops = تعلم إضافي + وسم منتجات إنستغرام.

### ✅ تأكيدات Loomer (تحقق حرفي)
- الهيكل الحديث: حملة واحدة + مجموعة واحدة (conversions، استهداف عريض، كل المواضع).
- للميزانيات الصغيرة: «consolidate your budget... into fewer targets».
- قاعدة الـ 50 نتيجة/أسبوع مذكورة كإرشاد لا قاعدة صارمة.
- فرّق بالكرييتف لا بالاستهداف ⟵ فصل الساعات يكون **بالكرييتف والقياس** قبل أن يكون بهيكل الحملات.

### ✅ تأكيدات سياسة الملكية الفكرية (كلها 3 أصوات تقريباً)
- كل الادعاءات الخمسة تأكدت + إضافتان: ميتا تتصرف **تحت عدم اليقين** («without an intellectual property report... we cannot know with certainty whether an ad infringes... we take action») ⟵ خطر إنفاذ خاطئ (false positive) على ساعات مشابهة الشكل حتى لو قانونية؛ وقناتا الإبلاغ الرسميتان: IP Help Center + برنامج Brand Rights Protection.

## خامساً: المتبقي للجلسة القادمة

1. ~~تحقق عدائي للادعاءات~~ ✅ أُنجز بالجولة المستأنفة (القسم رابعاً-ب) — بقي فقط ادعاءان ثانويان بلا حكم (عمود Variants بالـ UI، وحفظ تعريفات الفئات — والأخير صار غير ذي صلة بعد رفض ميزة category ads).
...
3. التوليف: إكمال `watches-vs-clothing-meta-strategy.md` — الخلاصة التنفيذية، جدول القرار النهائي، مثال feed CSV (ساعة: مينا × سوار مقابل بلوزة: لون × مقاس)، هيكلية الحملات والميزانية بشروط انتقال رقمية، قواعد أمان العلامات للساعات، خطة 90 يوم محدثة، المراجع. **تنبيه للتوليف:** استبدال أي ذكر لـ«category ads» بـ Product Sets (القسم رابعاً-ب).
4. **أولوية المستخدم المعلنة (2026-07-03): البدء تنفيذياً بالملابس (نساء + أطفال) — الساعات لاحقاً.** الدراسة تُكمَّل كمرجع قرار، والتنفيذ يتبع خطة الملابس أولاً.
5. الأرشيف الخام للجولة كاملة: `docs/research/` (journal.jsonl + سكريبت الـ workflow + مخرجات الجولة الأولى) — يغني عن إعادة أي بحث حتى لو راحت الجلسة.
